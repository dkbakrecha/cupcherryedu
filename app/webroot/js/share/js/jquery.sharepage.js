/* SharePage 1.1.1 by Dharmendra Bagrecha */
(function($) {
  ...
  var init = function(params) {
    return this.each(function(){
      ...
      $(this).prepend( "<div class='sharertitle sharecount' id='total-count'></div>" );
    });
  };
  ...
  var getTwitter = function(url, callback) {
    console.log("inside twitter");
    $.ajax({
      url: "http://public.newsharecounts.com/count.json?url=" + encodeURIComponent(url) + "&format=jsonp",
      dataType: "jsonp",
      success: function(data) {
        console.log(data);
        if (data) {
    ...
    buttons.forEach(function($button, index) {
      ...
      if (typeof networksLib[network].getShares === "function") {
        networksLib[network].getShares(url, function(count) {
          ...
          _tcount = _tcount + count;
          $(".sharecount").html("<span class='total-count'>" + _tcount + "</span> <span class='total-text'>Shares</span>" );
        });
      };
    });
  };
